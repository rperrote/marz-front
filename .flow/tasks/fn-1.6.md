...
- `src/shared/api/mutator.ts`:
  ...
  - Retry-on-401: si response es 401 y el `error.code ∈ {token_invalid, token_expired}` y no se ha reintentado aún:
...
- [ ] Unit test `src/shared/api/mutator.test.ts` (con `vi.mock` de fetch):
  ...
  - 401 + refresh devuelve null token → `signOut` invocado sin segundo fetch; verificar con spy que `fetch` se llamó exactamente 1 vez.
