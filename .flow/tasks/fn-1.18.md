...
- Mutation:
  ...
  - Zod parse con `CreatorOnboardingPayloadSchema` de `src/features/identity/onboarding/creator/schema.ts` (§D6) antes de enviar. Si falla, mostrar error genérico y navigate al primer paso con issue (no enviar request).
