...
describe('OnboardingTopbar', () => {
  it('renders step label', () => {
    render(<OnboardingTopbar stepLabel="Paso 3 de 10" />)
    expect(screen.getByText('Paso 3 de 10')).toBeInTheDocument()
  })

  it('renders exit button when onExit provided', async () => {
    const onExit = vi.fn()
    render(<OnboardingTopbar stepLabel="Paso 1 de 5" onExit={onExit} />)
  ...
  it('does not render exit button when onExit is not provided', () => {
    render(<OnboardingTopbar stepLabel="Paso 1 de 5" />)
    expect(screen.queryByText('Salir')).not.toBeInTheDocument()
  })

  it('is axe-clean', async () => {
    const { container } = render(
      <OnboardingTopbar stepLabel="Paso 1 de 5" onExit={() => {}} />,
    )
